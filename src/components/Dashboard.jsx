    ...
    <main className="birthday-page">
      ...
      <section className="shell-card page-scene">
        <div className="section-heading reveal reveal-1">
          ...
          <StoryText text="Take your time—there is a little something waiting at every stop." delay={150} speed={60} />
        </div>
        ...
        <div className="romantic-timeline-container reveal reveal-3">
          ...
          {progressItems.map((item, index) => {
            ...
            const content = (
              <>
                <div className={`timeline-node-circle ${item.active ? "is-active" : ""} ${isUnlocked ? "is-unlocked" : "is-locked"}`}>
                  {isUnlocked ? <span className="node-num">0{index + 1}</span> : <span className="node-lock">🔒</span>}
                </div>
            ...
            if (isUnlocked) {
              return (
                <Link key={item.path} to={item.path} className={`timeline-node ${isLabelOnTop ? "label-top" : ""}`} style={{ left: pos.left, top: pos.top }}>
                  {content}
                </Link>
              );
            }
            ...
            return (
              <div key={item.path} className={`timeline-node is-disabled ${isLabelOnTop ? "label-top" : ""}`} style={{ left: pos.left, top: pos.top }}>
                {content}
              </div>
            );
          })}
        ...
        <div className="panel wish-panel reveal reveal-4">
          ...
          <div className={`wish-text ${isFading ? "fade-out" : ""}`}>{wishes[currentWishIndex]}</div>
